...
public class Toms
{
                              ...
                              RefDouble w, RefDouble w1, RefInt ierr, boolean log_p)
    {
        ...
        if ((a_lt_b ? b : a) < eps * 0.001) {
            if (log_p) {
                if (a_lt_b) {
                    w.val = Math.log1p(-a / (a + b));
                    w1.val = Math.log(a / (a + b));
                } else {
                    w.val = Math.log(b / (a + b));
                    w1.val = Math.log1p(-b / (a + b));
                }
            } else {
                w.val = b / (a + b);
                w1.val = a / (a + b);
        ...
        if (Math.min(a, b) <= 1.0) {
            ...
            if (b0 < Math.min(eps, eps * a0)) {
                w.val = fpser(a0, b0, x0, eps, log_p);
                w1.val = log_p ? log1exp(w.val) : 0.5 - w.val + 0.5;
                // L_end
                if (do_swap) {
                    double t = w.val;
                    w.val = w1.val;
            ...
            if (a0 < Math.min(eps, eps * b0) && b0 * x0 <= 1.0) {
                ...
                // L_end_from_w1
                if (log_p) {
                    w.val = Math.log1p(-w1.val);
                    w1.val = Math.log(w1.val);
                } else {
                    w.val = 0.5 - w1.val + 0.5;
                }
                if (do_swap) {
                    double t = w.val;
                    w.val = w1.val;
            ...
            if (Math.max(a0, b0) > 1.0) {
                if (b0 <= 1.0) {
                    // L_w_bpser
                    w.val = bpser(a0, b0, x0, eps, log_p);
                    w1.val = log_p ? log1exp(w.val) : 0.5 - w.val + 0.5;
                    if (do_swap) {
                        double t = w.val;
                        w.val = w1.val;
                ...
                if (x0 >= 0.29) {
                    // L_w1_bpser
                    w1.val = bpser(b0, a0, y0, eps, log_p);
                    w.val = log_p ? log1exp(w1.val) : 0.5 - w1.val + 0.5;
                    if (do_swap) {
                        double t = w.val;
                        w.val = w1.val;
                        w1.val = t;
                ...
                if (b0 > 15.0) {
                    ...
                    if (w1.val == 0.0 || (0.0 < w1.val && w1.val < Double.MIN_VALUE)) {
                        ...
                        if (log_p) {
                            ...
                            w.val = -Math.expm1(w1.val);
                            w1.val = Math.exp(w1.val);
                        }
                        if (do_swap) {
                            double t = w.val;
                            w.val = w1.val;
            ...
            if (b0 < 40.0) {
                if (b0 * x0 <= 0.7 || (log_p && lambda > 650.0)) {
                    ...
                    if (w.val <- Double.MIN_VALUE && log_p) {
                        b0 += n;
                        w.val = bpser(a0, b0, x0, eps, log_p);
                        w1.val = log_p ? log1exp(w.val) : 0.5 - w.val + 0.5;
                        if (do_swap) {
                ...
                if (b0 <= 100.0 || lambda > b0 * 0.03) {
                    // L_bfrac
                    w.val = bfrac(a0, b0, x0, y0, lambda, eps * 15.0, log_p);
                    w1.val = log_p ? log1exp(w.val) : 0.5 - w.val + 0.5;
                    if (do_swap) {
                        double t = w.val;
                        w.val = w1.val;
            ...
            w.val = basym(a0, b0, lambda, eps * 100.0, log_p);
            w1.val = log_p ? log1exp(w.val) : 0.5 - w.val + 0.5;
            if (do_swap) {
                double t = w.val;
                w.val = w1.val;
    ...
    {
        return x > -Dpq.M_LN2 ? Math.log(-rexpm1(x)) : Math.log1p(-Math.exp(x));
    }

    private static double fpser(double a, double b, double x, double eps, boolean log_p)
    {
        /* ----------------------------------------------------------------------- *
    ...
    private static double apser(double a, double b, double x, double eps)
    {
        /* -----------------------------------------------------------------------
    ...
    private static double bpser(double a, double b, double x, double eps, boolean log_p)
    {
        /* -----------------------------------------------------------------------
        ...
        if(Math.abs(w) > tol) { // the series did not converge (in time)
            // warn only when the result seems to matter:
            if(( log_p && !(a*sum > -1. && Math.abs(Math.log1p(a * sum)) < eps*Math.abs(ans))) ||
                    (!log_p && Math.abs(a*sum + 1.) != 1.))
                System.out.printf(
                        " bpser(a=%g, b=%g, x=%g,...) did not converge (n=1e7, |w|/tol=%g > 1; A=%g)",
                        a,b,x, Math.abs(w)/tol, ans);
        }

        if(log_p) {
            if (a*sum > -1.) ans += Math.log1p(a * sum);
            else {
    ...
    private static double bup(double a, double b, double x, double y, int n, double eps,
                              boolean give_log)
    {
        /* ----------------------------------------------------------------------- */
        ...
        /* L10: */
        ret_val = give_log
                ? brcmp1(mu, a, b, x, y, true) - Math.log(a)
	...
	/* -----------------------------------------------------------------------
	   ...
	   -----------------------------------------------------------------------*/
        ...
        if(Double.isInfinite(lambda)) return Double.NaN;// TODO: can return 0 or 1 (?)

        brc = brcomp(a, b, x, y, log_p);
        if(Double.isNaN(brc)) { // e.g. from   L <- 1e308; pnbinom(L, L, mu = 5)
            System.out.println("Argument out of domain in bfrac.");
            return Double.NaN;
        }
        if (!log_p && brc == 0.) {
        ...
        if(n >= 10000 && Math.abs(r - r0) > eps * r)
            System.out.printf(
    ...
    private static double brcomp(double a, double b, double x, double y, boolean log_p)
    {
        ...
        if (a0 < 8.) {
            ...
            return(log_p
                    ? -M_LN_SQRT_2PI + .5*Math.log(b * x0) + z - bcorr(a,b)
                    : const__ * Math.sqrt(b * x0) * z * Math.exp(-bcorr(a, b)));
        }
    } /* brcomp */

    // called only once from  bup(),  as   r = brcmp1(mu, a, b, x, y, FALSE) / a;
    private static double brcmp1(int mu, double a, double b, double x, double y, boolean give_log)
    {
        /* -----------------------------------------------------------------------
        ...
        if (a0 < 8.) {
            ...
            if (b0 >= 8.) {
                ...
                // L50:
                c = give_log
                        ? Math.log1p(gam1(a)) + Math.log1p(gam1(b)) - Math.log(z)
                        : (gam1(a) + 1.) * (gam1(b) + 1.) / z;
            ...
            if (n >= 1) {
                ...
                u += Math.log(c); // TODO?: log(c) = log( prod(...) ) =  sum( log(...) )
            }
            // L70:
            z -= u;
            b0 += -1.;
            ...
            // L72:
            return give_log
                    ? Math.log(a0)+ esum(mu, z, true) + Math.log1p(gam1(b0)) - Math.log(t) // TODO? log(t) log1p(..)
            ...
            double lx0 = -Math.log1p(b/a); // in both cases
            ...
            return give_log
                    ? Math.log(const__)+ (Math.log(b) + lx0)/2. + z      - bcorr(a, b)
                    :     const__ * Math.sqrt(b * x0)      * z * Math.exp(-bcorr(a, b));
        }

    } /* brcmp1 */

    private static void bgrat(double a, double b, double x, double y, RefDouble w,
                              double eps, RefInt ierr, boolean log_w)
    {
        /* -----------------------------------------------------------------------
         ...
         * Didonato & Morris(1992), p.362 */
                lnx = (y > 0.375) ? Math.log(x) : alnrel(-y),
        ...
        double
                ...
                log_r = Math.log(b) + Math.log1p(gam1(b)) + b * Math.log(z) + nu * lnx,
                // FIXME work with  log_u = log(u)  also when log_p=FALSE  (??)
                // u is 'factored out' from the expansion {and multiplied back, at the end}:
                log_u = log_r - (algdiv(b, a) + b * Math.log(nu)),// algdiv(b,a) = log(gamma(a)/gamma(a+b))
        ...
        if (log_u == Double.NEGATIVE_INFINITY) {
            ...
            /* L_Error:    THE EXPANSION CANNOT BE COMPUTED */ ierr.val = 2; return;
        }
        ...
        double l = // := *w/u .. but with care: such that it also works when u underflows to 0:
                log_w
                        ? ((w.val == Double.NEGATIVE_INFINITY) ? 0. : Math.exp(w.val - log_u))
                        : ((w.val == 0.)        ? 0. : Math.exp(Math.log(w.val) - log_u));
        ...
        for (int n = 1; n <= n_terms_bgrat; ++n) {
            ...
            if (sum <= 0.) {
                ...
                /* L_Error:    THE EXPANSION CANNOT BE COMPUTED */ ierr.val = 3; return;
            }
    ...
    private static double grat_r(double a, double x, double log_r, double eps)
    {
        /* -----------------------------------------------------------------------
         ...
         * ----------------------------------------------------------------------- */
        final double M_SQRT_PI = 1.772453850905516027298167483341;	/* sqrt(pi) */
        if (a * x == 0.) { /* L130: */
            if (x <= a) {
                /* L100: */ return Math.exp(-log_r);
            } else {
                /* L110:*/  return 0.;
            }
        }
            ...
            if ((x >= 0.25 && (a < x / 2.59)) || (z > -0.13394)) {
                ...
                return /* q/r = */ (0.5 - p + 0.5) * Math.exp(-log_r);
            }
    ...
    private static double basym(double a, double b, double lambda, double eps, boolean log_p)
    {
        /* ----------------------------------------------------------------------- */
    ...
    private static double exparg(int l)
    {
        /* --------------------------------------------------------------------
        ...
        int m = (l == 0) ? 1024 : -1024 - 1;

        return m * lnb * 0.99999;
    } /* exparg */

    private static double esum(int mu, double x, boolean give_log)
    {
        /* ----------------------------------------------------------------------- */
        ...
        // else :
        double w;
        double ret = Math.exp((double) mu) * Math.exp(x);
        if (x > 0.) { /* L10: */
            if (mu > 0)  return ret;
            w = mu + x;
            if (w < 0.) return ret;
    ...
    private static double alnrel(double a)
    {
        /* -----------------------------------------------------------------------
    ...
    private static double erf__(double x)
    {
        /* -----------------------------------------------------------------------
        ...
        if (ax <= 4.) { //  |x| in (0.5, 4]
            ...
            bot = ((((((q[0] * ax + q[1]) * ax + q[2]) * ax + q[3]) * ax + q[4]) * ax
                    + q[5]) * ax + q[6]) * ax + q[7];
            double ret = 0.5 - Math.exp(-x * x) * top / bot + 0.5;
            return (x < 0) ? -ret : ret;
        }
        ...
        t = (c - top / (x2 * bot)) / ax;
        double ret = 0.5 - Math.exp(-x2) * t + 0.5;
        return (x < 0) ? -ret : ret;
    } /* erf */

    private static double erfc1(int ind, double x)
    {
        /* ----------------------------------------------------------------------- */
    ...
    private static double gam1(double a)
    {
        /*     ------------------------------------------------------------------ */
        ...
        if (d > 0.)
            t = d - 0.5;
        if (t < 0.) { /* L30: */
            double[] r = { -.422784335098468,-.771330383816272,
    ...
    private static double psi(double x)
    {
        ...
        /*     PSI(X) / (X - X0),  0.5 <= X <= 3. */
        double[] p1 = { .0089538502298197,4.77762828042627,
                142.441585084029,1186.45200713425,3633.51846806499,
                4138.10161269013,1305.60269827897 };
        double[] q1 = { 44.8452573429826,520.752771467162,
	 ...
	 * Originally:  xmax1 = amin1(ipmpar(3), 1./spmpar(1))  */
        ...
        d2 = 1.0 / 2.2204460492503131E-16; /*= 0.5 / (0.5 * DBL_EPS) = 1/DBL_EPSILON = 2^52 */
    ...
    private static double betaln(double a0, double b0)
    {
        /* -----------------------------------------------------------------------
    ...
    private static double gsumln(double a, double b)
    {
        /* ----------------------------------------------------------------------- */
    ...
    private static double algdiv(double a, double b)
    {
        /* ----------------------------------------------------------------------- */
